...
class DTC(nn.Module):
    ...
    def sim(self, f):
        ...
        q = compute_similarity(f, self.cluster.cuda(), self.similarity)
        q = torch.pow(1 + q, -1)
        return q / torch.sum(q, dim=-1).unsqueeze(1)
    
    def init_centroids(self, data_loader):
        with torch.no_grad():
            Z = []
            for f, _,_ in data_loader:
                z = self(f.float().cuda())[:, -1, :].squeeze().cpu().detach().numpy()
                Z.append(z)
            Z = np.concatenate(Z)
        

        cluster = KMeans(n_clusters = self.cluster_num, random_state = 0, n_init=20).fit(Z)
    ...
    def loss(self, x):
        ...
        q = self.sim(f)
        p = self.p(q)
        loss1 = torch.sum(p * torch.log((p / (q + 1e-6)) + 1e-6)) / p.shape[0]
